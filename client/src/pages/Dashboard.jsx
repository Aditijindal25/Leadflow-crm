...
export default function Dashboard() {
  ...
  useEffect(() => { listLeads({ limit: 8 }).then((data) => { setLeads(data.data); setMeta(data.meta); }).catch((requestError) => setError(requestError.message)).finally(() => setLoading(false)); }, []);
  if (!isAuthenticated) return <Navigate to="/login" replace />;
  const counts = ['NEW', 'CONTACTED', 'QUALIFIED', 'PROPOSAL', 'NEGOTIATION', 'CONVERTED', 'LOST', 'ON_HOLD'].map((status) => ({ status, count: leads.filter((lead) => lead.status === status).length }));
