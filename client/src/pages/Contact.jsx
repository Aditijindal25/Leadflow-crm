...
export default function Contact() {
  ...
  return <main className="shell"><nav className="nav"><Link className="brand" to="/">LEADFLOW <span>CRM</span></Link><div className="nav-links"><button className="theme-toggle" type="button" onClick={toggleTheme}>{theme === 'dark' ? '☀ Light' : '☾ Dark'}</button><Link className="button button-dark" to="/">Back home</Link></div></nav><section className="form-layout"><div><p className="eyebrow">Add new lead</p><h1>Capture a new opportunity.</h1><p className="hero-copy">Add the details your team needs to qualify and follow up with this lead.</p></div><form className="panel lead-form" onSubmit={submit}>
    ...
    {state.success && <p className="notice success" role="status">Lead created successfully.</p>}{state.error && <p className="notice error" role="alert">Something went wrong while creating this lead. Please try again.</p>}<button className="button button-primary submit-button" disabled={state.loading}>{state.loading ? 'Creating lead...' : 'Create lead'}</button>
  </form></section></main>;
}
